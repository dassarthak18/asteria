...
fn load_iris(path: &str) -> (Vec<Vec<f32>>, Vec<usize>) {
    let content = std::fs::read_to_string(path).expect("cannot read iris.data");
    ...
    for line in content.lines() {
        ...
        let feats: Vec<f32> = parts[..4].iter().map(|s| s.trim().parse::<f32>().unwrap()).collect();
        let label = match parts[4].trim() {
            "Iris-setosa"     => 0,
            "Iris-versicolor" => 1,
            _                 => 2,
        };
...
fn main() {
    ...
    net.add_layer(DenseLayer::new("h0".to_string(), 16, ActivationType::Relu,
    ...
    println!("=== Iris: Softmax Classifier (CosineAnnealing LR) ===");
